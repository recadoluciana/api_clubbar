...
def dashboard_superadmin(db: Session = Depends(get_db)):
    ...
    vendas_hoje = (
        db.query(Venda)
        .filter(Venda.dtcriacao >= hoje_inicio)
        .filter(Venda.dtcriacao <= hoje_fim)
        .count()
    )

    valor_vendas_hoje = (
        db.query(func.coalesce(func.sum(Venda.vrtotalvenda), 0))
        .filter(Venda.dtcriacao >= hoje_inicio)
        .filter(Venda.dtcriacao <= hoje_fim)
        .scalar()
    )
    ...
    return {
        ...
        "valor_vendas_hoje": float(valor_vendas_hoje or 0),
    }